...
function normalizeClearKey(rawKey) {
    ...
    try {
        let parsed = typeof rawKey === 'string' ? JSON.parse(rawKey) : rawKey;
    ...
    try {
        ...
        shakaPlayer.addEventListener('error', event => {
            ...
            if (playerError) {
                ...
                playerError.innerText = `Playback error: ${event.detail?.code || 'Unknown error'}`;
            }
        });
        ...
        if (playerError) {
            playerError.style.display = 'block';
            playerError.innerText = 'Unable to play this stream in the browser.';
        }
    }
}
